...
> Historical implementation notes. The latest video-generation changes are documented in `HIGH-QUALITY-VIDEO-UPGRADE.md`.
...
- Added itemized provider-cost events and an admin dashboard for real monthly
  ...
  images, video seconds, and narration seconds).
...
- Enforced supported 8-second scene increments on the server (current primary presets: 8, 32, and 64 seconds) and
  kept checkout eligibility tied to the actual credits required by the chosen
  duration, mode, narration, and quality.
- Made payment credit grants durable and idempotent. Returning from a
...
## Historical verification (before the 2026-08-27 frontend rebuild)

- TypeScript type checks: passed
- API automated tests: 75 passed, 0 failed
- Production frontend and API builds: passed

For the current frontend rebuild and its sandbox validation status, see `FRONTEND-UX-REBUILD.md`.
