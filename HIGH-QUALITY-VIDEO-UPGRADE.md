...
- **Premium Veo by default:** `veo-3.1-generate-preview` is now the default final-render model. An old Fast model setting is ignored unless the operator explicitly selects the draft render profile.
...
AI_VIDEO_RENDER_PROFILE=final
```

To intentionally use a Fast model for internal testing, set both its model name
and `AI_VIDEO_RENDER_PROFILE=draft`. Customer renders stay on the final profile.
