...
import {
  addJobMessage,
  assertPaidProviderAuthorization,
  claimJob,
  claimRenderAndSpend,
...
async function copyCaptureFiles(sourceJobId: string, targetJobId: string) {
  ...
  // A reused project may be opened after the VPS cache was cleared/restarted.
  ...
  await Promise.all([...referencedNames].map((name) => ensureLocalAsset(sourceJobId, name).catch(() => null)));
  ...
  await Promise.all(
    names
      ...
      .map(async (name) => {
        const targetPath = path.join(toDir, name);
        await fs.copyFile(path.join(fromDir, name), targetPath);
        await uploadFileToR2(targetJobId, name, targetPath);
      }),
  );
}
...
async function loadReferenceCaptures(
  ...
  const firstMediaLabel =
    meta?.pages?.[0]?.title || (uploadedMedia ? "Uploaded reference photo 1" : "Real desktop homepage full page");
  ...
  await add("screenshot.jpg", uploadedMedia ? "Uploaded reference photo 1" : "Real desktop homepage viewport");
  ...
  const isInteractionPage = (pageMeta: { screenshotUrl?: string }) => {
    try {
      return /^interaction-/.test(path.basename(new URL(pageMeta.screenshotUrl ?? "", "http://local").pathname));
    } catch {
      return false;
    }
  };
  ...
  for (const [index, pageMeta] of orderedRest.entries()) {
    ...
    const detail =
      pageMeta?.title ||
      pageMeta?.url ||
      (uploadedMedia ? `reference photo ${index + 2}` : `internal page ${index + 2}`);
    await add(filename, uploadedMedia ? String(detail) : `Real captured ${detail}`);
  }
  ...
  try {
    const iconPath = await ensureLocalAsset(jobId, "website-icon.jpg");
    if (iconPath) websiteIcon = await fs.readFile(iconPath);
  } catch {
    /* optional */
  }
  ...
  if (websiteIcon?.length)
    pageCaptures.push({
      id: "website-icon.jpg",
      label: "Real captured WEBSITE ICON / LOGO — use for brand identity and the closing card",
      buffer: websiteIcon,
    });
  return pageCaptures;
}
...
router.get("/:id", tryAuth, async (req, res) => {
  try {
    ...
    const response: JobStatusResponse = {
      ...
      errorMessage: showTechnicalErrors ? job.error_message : publicJobErrorMessage(job.error_message),
      assets: assets.map((a) => ({
        id: a.id,
      ...
      messages: messages.map((message) => {
        const resultAssets =
          message.kind === "result"
            ? (assetsByJob.get(message.job_id) ?? []).filter(
...
router.post("/:id/reuse", requireAuth, async (req, res) => {
  try {
    ...
    await addJobMessage(job.id, "assistant", continuationLabel, "source_continuation", { sourceJobId: source.id });
  ...
  try {
    ...
    void (async () => {
      try {
        ...
        try {
          const screenshotPath = await ensureLocalAsset(job.id, "screenshot.jpg");
          if (screenshotPath) {
        ...
        try {
          const fullScreenshotPath = await ensureLocalAsset(job.id, "screenshot-full.jpg");
          if (fullScreenshotPath) {
            const buf = await fs.readFile(fullScreenshotPath);
        ...
        // Credits were atomically reserved before this point. Re-read the
        // authoritative job immediately before the first paid provider call;
        ...
        console.info(
          `[storyboard] plan ready job=${job.id} requested_mode=${mode} planner_mode=${plannerMode} beats=${storyboard.scenes.length} refs=${storyboard.selectedCaptureIds?.length ?? 0}`,
        );
        if (aiError) {
          await addJobMessage(
            job.id,
        ...
        await refundJobCredits(job.id, req.user!.id, planningQuote.totalCredits, `Planning failed ${job.id}`).catch(
          () => {},
        );
        await updateJob(job.id, {
          status: "failed",
...
router.post("/:id/quote", requireAuth, async (req, res) => {
  try {
    const input = z
      .object({
        audioMode: z.enum(["voice_music", "native_audio", "music_only", "silent"]).optional().default("voice_music"),
      })
      .parse(req.body);
    const job = await getJob(String(req.params.id));
    if (!job || job.deleted_at || job.user_id !== req.user!.id) throw new AppError("Job not found.", 404, "NOT_FOUND");
...
router.post("/:id/render", requireAuth, async (req, res) => {
  try {
    ...
    void (async () => {
      ...
      try {
        // The render claim above atomically settled the full price. Verify the
        // reservation again immediately before launching narration, image or
        // video providers so retries/direct API calls cannot run unfunded.
        await assertPaidProviderAuthorization(job.id, req.user!.id, cost, "rendering");
        ...
        const siteTitle = meta?.title ?? (job.mode === "custom" ? "Custom idea" : new URL(job.source_url).hostname);
        ...
        const narrationPromise: Promise<string | null> = wantsNarration
          ? (async () => {
              try {
                ...
                const narration = await synthesizeVoiceover(job.id, script, 0, effectiveVideoMode);
                console.info(
        ...
        const videoPromise: Promise<PromiseSettledResult<Awaited<ReturnType<typeof generateMarketingVideo>>>> =
          wantsVideo
                ...
                (pct, message, providerEtaSeconds) => {
                  ...
                  if (pct <= 81) {
                    ...
                    const remainingSceneSeconds =
                      pct >= 94 ? 0 : (elapsedSeconds / sceneFraction) * (1 - sceneFraction);
        ...
        for (const result of photoResults) {
          if (result.status === "fulfilled") {
            await createAsset(job.id, "photo", result.value.url, result.value.aspectRatio, false, true);
            photoCount++;
          }
        }
        ...
        if (photoCount === 0 && !hasVideo) {
          ...
          const videoErr = wantsVideo
            ? ((videoResult as PromiseRejectedResult).reason as Error | undefined)
            : undefined;
          const parts = [
