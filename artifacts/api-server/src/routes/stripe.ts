...
const PRODUCTS = {
  ...
  single8: { env: 'STRIPE_PRICE_SINGLE_8', mode: 'payment', credits: 8, plan: 'creator' },
  single30: { env: 'STRIPE_PRICE_SINGLE_30', mode: 'payment', credits: 24, plan: 'creator' },
  single60: { env: 'STRIPE_PRICE_SINGLE_60', mode: 'payment', credits: 56, plan: 'creator' },
...
router.post('/checkout', requireAuth, async (req, res) => {
  try {
    ...
    const { plan } = z.object({ plan: z.enum(ids) }).parse(req.body);
    ...
    await once(event.id, async () => {
      if (event.type === 'checkout.session.completed') {
        ...
        if (session.mode === 'payment' && session.payment_status === 'paid') {
          ...
          await query(`UPDATE users SET credits_balance=credits_balance+$1, plan=CASE WHEN plan='free' THEN $2 ELSE plan END, updated_at=NOW() WHERE id=$3`, [credits, plan, userId]);
          await query('INSERT INTO credit_transactions (user_id, delta, reason) VALUES ($1,$2,$3)', [userId, credits, `Stripe purchase ${session.id}`]);
        }
      }
      ...
      if (event.type === 'invoice.paid') {
        ...
        await query('UPDATE users SET plan=$1, credits_balance=credits_balance+$2, updated_at=NOW() WHERE id=$3', [plan, credits, userId]);
        ...
        await query('INSERT INTO credit_transactions (user_id, delta, reason) VALUES ($1,$2,$3)', [userId, credits, `Subscription renewal ${invoice.id}`]);
      }
