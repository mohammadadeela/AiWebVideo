...
async function accessToken() {
  if (accessTokenCache && accessTokenCache.expiresAt > Date.now() + 30_000) return accessTokenCache.value;
...
async function paypalRequest(path: string, init: { method: string; idempotencyKey?: string }) {
  ...
  try {
    ...
    logger.error({ err: error, path }, '[paypal-card-settlement] provider request failed');
    throw new AppError('The payment service is temporarily unavailable.', 502, 'PAYMENT_PROVIDER_UNAVAILABLE');
  }
  ...
  if (!response.ok) {
    logger.warn({ status: response.status, path }, '[paypal-card-settlement] provider rejected request');
...
/**
 * Embedded Card Fields responses do not always repeat purchase_units[].custom_id.
 * PayPal can instead return custom_id on the completed capture. The local pending
 * payment row is already bound to both order id and authenticated user, so a
 * missing remote copy is safe; a present-but-different remote copy is never safe.
 */
export function validateEmbeddedCompletedOrder(
  ...
  const capturesValue = (units[0]?.payments as { captures?: unknown } | undefined)?.captures;
  const captures = Array.isArray(capturesValue)
    ? capturesValue.filter((capture): capture is Record<string, unknown> => Boolean(capture) && typeof capture === 'object')
    : [];
  ...
  const unitCustomId = typeof units[0]?.custom_id === 'string' ? units[0].custom_id : '';
  const captureCustomId = completed.find((capture) => typeof capture.custom_id === 'string')?.custom_id;
  const remoteUserId = unitCustomId || (typeof captureCustomId === 'string' ? captureCustomId : '');
  if (remoteUserId && remoteUserId !== expected.userId) {
...
async function sendReceiptOnce(orderId: string, payment: PendingPayment) {
  ...
  const inserted = await query(
    ...
    [notificationKey, payment.user_id],
  );
...
async function saveVaultMetadata(userId: string, captured: Record<string, unknown>) {
  ...
  const source = captured.payment_source as { card?: Record<string, unknown> } | undefined;
...
async function finalizeEmbeddedOrder(userId: string, orderId: string) {
  ...
  try {
    completed = await paypalRequest(`/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, {
      method: 'POST',
