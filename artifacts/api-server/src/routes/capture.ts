...
import { addJobMessage, createAsset, createJob, updateJob } from '../lib/queries.js';
  ...
  try {
    ...
    const runCapture = async () => {
      try {
        ...
        const timedOut = (err as Error).message === 'CAPTURE_TIMEOUT';
        await updateJob(job.id, {
          status: 'failed',
          progress: 0,
          status_message: timedOut ? 'Website took too long to respond' : 'Website capture paused',
          eta_seconds: 0,
