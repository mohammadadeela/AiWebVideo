...
export async function saveImageFile(jobId: string, name: string, data: Buffer, persistToR2 = true): Promise<string> {
  const dir = path.join(ASSETS_DIR, jobId);
  await ensureDir(dir);
  await fs.writeFile(path.join(dir, name), data);
  if (persistToR2) await uploadBufferToR2(jobId, name, data);
