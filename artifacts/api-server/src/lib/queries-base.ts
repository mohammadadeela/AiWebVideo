...
export async function createAsset(
  ...
  // Browser captures uploaded through saveImageFile() are already persisted.
  // Rendered videos/photos and recordings are produced directly on disk, so
  // make them durable in R2 before the database begins advertising the asset.
  if (type !== 'screenshot') await persistAssetUrlToR2(storageUrl);
  const { rows } = await query<AssetRow>(
