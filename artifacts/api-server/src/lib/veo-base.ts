...
// Speed is the public default because Veo generation is the dominant
// latency in a customer production. Fast is still the same Veo 3.1 family and
// Google documents it as optimized for speed/business use cases. Operators can
// explicitly select Standard for maximum quality with AI_VIDEO_SPEED_MODE=quality.
export function geminiModelChain(): string[] {
  const pinned = process.env.GEMINI_VIDEO_MODEL?.trim();
  const speedMode = process.env.AI_VIDEO_SPEED_MODE?.trim().toLowerCase() || 'fast';
