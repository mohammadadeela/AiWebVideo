 ...
 * `concurrency` defaults to the legacy scene-rendering concurrency, while the
 * continuous-film path passes 1 because every Veo continuation depends on the
 * previous continuation and therefore runs strictly sequentially.
 */
export function totalGenerationTimeoutMs(operationCount: number, concurrency = VIDEO_CONCURRENCY): number {
  ...
  const batches = Math.max(1, Math.ceil(Math.max(1, operationCount) / lanes));
...
/**
 * Veo accepts at most three asset references for the continuous base. Prefer
 * the captures the AI storyboard actually selected, in story order, then fill
 * any empty slots deterministically. The old first-three rule could send pages
 * unrelated to the customer's requested subject while omitting the chosen one.
 */
export function selectContinuousReferenceIndices(
  scenes: Array<Pick<StoryboardScene, 'sourceIndices'>>,
  referenceCount: number,
  limit = 3,
) {
  const selected: number[] = [];
  const add = (value: number) => {
    if (!Number.isInteger(value) || value < 0 || value >= referenceCount || selected.includes(value)) return;
    selected.push(value);
  };
  for (const scene of scenes) {
    for (const value of scene.sourceIndices ?? []) {
      add(value);
      if (selected.length >= limit) return selected;
    }
  }
  for (let index = 0; index < referenceCount && selected.length < limit; index += 1) add(index);
  return selected;
}
...
/**
 * Transient errors are only retryable while POLLING an already-submitted Veo
 * operation. They are deliberately never used to create a second generation
 * request. A duplicate generation can be billable even when the first request
 * later succeeds, so production safety takes priority over automatic recovery.
 */
export function isRetryableGeminiError(message: string): boolean {
  return /"code"\s*:\s*(?:14|429|503)|RESOURCE_EXHAUSTED|UNAVAILABLE|high demand|spikes in demand|model (?:is )?(?:overloaded|busy)|ECONNRESET|ETIMEDOUT|Input video must be a video that was generated by VEO that has been processed/i.test(message);
}

// Customer renders are final-quality by default. In particular, an old Fast
// model value left in GEMINI_VIDEO_MODEL must not silently downgrade paid
// output. Fast is honored only when the operator also opts into the explicit
// draft profile; non-Fast custom/future models remain valid explicit choices.
export function geminiModelChain(): string[] {
  const pinned = process.env.GEMINI_VIDEO_MODEL?.trim();
  const renderProfile = process.env.AI_VIDEO_RENDER_PROFILE?.trim().toLowerCase() || 'final';
  if (pinned && (!/(?:^|[-_.])(?:fast|lite)(?:$|[-_.])/i.test(pinned) || renderProfile === 'draft')) {
    return [pinned];
  }
...
async function generateGeminiScene(
  ...
  // Never switch models automatically after a generation failure. A model
  // switch is another paid generation attempt for the same scene.
  return generateGeminiSceneWithModel(
...
async function generateGeminiSceneWithModel(
  ...
  // images as Veo 3.1 asset references.
  ...
  if (Date.now() >= (deadlineAt ?? Number.POSITIVE_INFINITY)) {
    throw new Error(`AI video generation exceeded the overall production timeout before scene ${sceneIndex + 1}.`);
  }
  ...
  try {
    ...
    throw new Error(`Veo could not start scene ${sceneIndex + 1}. Automatic generation retries are disabled to prevent duplicate provider charges. ${message}`);
  }
  ...
  while (!operation.done) {
    ...
    try {
      ...
      // Poll the SAME accepted operation again. This does not submit a new
      // generation and therefore cannot duplicate the provider charge.
      ...
      onStatus?.(`Connection interrupted — continuing the same AI scene ${sceneIndex + 1}`);
    }
  }

  if (operation.error) {
    const operationError = JSON.stringify(operation.error);
    throw new Error(`Gemini video generation failed for scene ${sceneIndex + 1}. Production stopped without an automatic retry to protect provider spend. ${operationError}`);
  }
...
export async function finishAudio(
  ...
  if (narrationPath) {
    // TTS timing can legitimately run longer than the scripted target. Exact-
    // duration deliveries gently speed up that audio so every spoken word fits;
    // callers without an exact target retain the legacy frozen-frame behavior.
    const [videoSeconds, narrationSeconds] = await Promise.all([
      duration(source),
      duration(narrationPath).catch(() => 0),
    ]);
    const exactSeconds = Number.isFinite(exactDurationSeconds) && Number(exactDurationSeconds) > 0
      ? Number(exactDurationSeconds)
      : null;
    const fitNarration = exactSeconds !== null && narrationSeconds > exactSeconds + 0.25;
    const extendSeconds = exactSeconds === null && narrationSeconds > videoSeconds + 0.25 ? narrationSeconds - videoSeconds : 0;
    ...
    const targetSeconds = (exactSeconds ?? Math.max(videoSeconds, narrationSeconds)).toFixed(2);
    const narrationSpeed = fitNarration ? narrationSeconds / Number(targetSeconds) : 1;
    const narrationTempoFilters: string[] = [];
    let remainingNarrationSpeed = narrationSpeed;
    while (remainingNarrationSpeed > 2) {
      narrationTempoFilters.push('atempo=2');
      remainingNarrationSpeed /= 2;
    }
    if (fitNarration) narrationTempoFilters.push(`atempo=${remainingNarrationSpeed.toFixed(6)}`);
    const narrationTiming = fitNarration ? `${narrationTempoFilters.join(',')},atrim=duration=${targetSeconds},` : '';
    ...
    const filters = [
      ...
      nativeAudioPresent
        ? `[0:a]volume=0.32,apad=whole_dur=${targetSeconds}[bed];[1:a]${narrationTiming}volume=1.0,apad=whole_dur=${targetSeconds}[voice];[bed][voice]amix=inputs=2:duration=first:dropout_transition=2[a]`
        : `[1:a]${narrationTiming}volume=1.0,apad=whole_dur=${targetSeconds}[a]`,
    ];
    await execFileAsync('ffmpeg', [
      ...
      '-shortest', ...(exactSeconds !== null ? ['-t', exactSeconds.toFixed(3)] : []), '-movflags', '+faststart', output,
    ], { timeout: 10 * 60_000, maxBuffer: 8 * 1024 * 1024 });
    if (fitNarration) {
      console.info(`[ai-video] narration ${narrationSeconds.toFixed(1)}s fitted into the exact ${targetSeconds}s delivery without dropping spoken content`);
...
export async function addBrandClosingOverlay(source: string, output: string, jobId: string, siteTitle: string) {
  const iconPath = await ensureLocalAsset(jobId, 'website-icon.jpg') ?? path.join(ASSETS_DIR, jobId, 'website-icon.jpg');
...
/**
 * Submit exactly ONE provider generation for a base/continuation and then poll
 * that accepted operation until it settles. Poll transport failures may retry
 * the same operation; provider generation itself is never automatically
 * resubmitted. This is the money-safety boundary for every continuous film.
 */
...
async function masterContinuousVideo(
  ...
  args.push(
    ...
    '-vf', filter,
    // CRF controls delivery quality. Medium keeps the same visual target while
    // avoiding the long CPU-only "slow" finishing pass users were waiting on.
...
export async function generateMarketingVideo(
  ...
  try {
    ...
    // Continuous extensions are strictly sequential: extension N cannot start
    // until extension N-1 has produced the provider video reference. Giving
    // this path a concurrency of 1 prevents legitimate 48/96/144s renders from
    // being killed by a timeout calculated as if extensions ran in parallel.
    const jobTotalTimeoutMs = totalGenerationTimeoutMs(operationCount, 1);
