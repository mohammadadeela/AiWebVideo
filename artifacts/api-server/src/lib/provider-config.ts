...
export async function resolveProvider(kind: MediaKind): Promise<'gemini' | 'open_source'> {
  ...
  const resolved = (value: 'gemini' | 'open_source') => {
    console.info(`[provider] feature=${kind} requested=${selected} resolved=${value}`);
    return value;
  };
  if (selected === 'gemini') {
  ...
  if (selected === 'open_source') {
    ...
    return resolved('open_source');
  }
  // In Auto mode, prefer Gemini when configured. For video this selects the
  // native-audio, first/last-frame AI path used for grounded website actions;
  // admins can still explicitly choose open_source for cost/throughput.
  if (availability.gemini) return resolved('gemini');
  if (availability.openSource) return resolved('open_source');
