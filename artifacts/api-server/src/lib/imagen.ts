...
- The customer's complete request is the highest-priority art direction. Account for every compatible detail about subject/product, action, setting, composition, style, camera/lens, light, colors, materials, exclusions, aspect ratio and intended use. Do not silently replace specifics with a generic template.
...
- Do not generate visible typography by default. No random text, fake prices, fake product names, fake letters, misspelled logos, watermarks or unrelated brand marks.
- If the customer explicitly requests visible copy, interpret the requested meaning but render only short, natural, correctly spelled ENGLISH. Never synthesize Arabic, Korean, Cyrillic, Chinese, another non-English script, or pseudo-language glyphs. Brand names/proper names may remain unchanged only when their exact real spelling is supplied.
- Treat all text already inside a reference image as protected source pixels. Never redraw, translate, retype or "improve" it. If it cannot stay pixel-faithful, omit or de-emphasize the text-bearing area rather than inventing lettering.
...
async function masterGeneratedImage(
  ...
  const { stdout: sourceDimensions } = await execFileAsync('ffprobe', [
    '-v', 'error', '-select_streams', 'v:0', '-show_entries', 'stream=width,height',
    '-of', 'csv=s=x:p=0', source,
  ], { timeout: 30_000, maxBuffer: 1 * 1024 * 1024 });
  if (sourceDimensions.trim() === `${width}x${height}`) {
    // The provider already returned the exact requested canvas. Preserve its
    // pixels and skip an unnecessary image re-encode.
...
async function runImageGeneration(
  ...
  const geminiImageModel = imageModelName();
  console.info(
  ...
  await saveImageFile(jobId, rawFilename, Buffer.from(image.data, 'base64'), false);
...
- No on-screen text, captions, logos, or watermarks unless the idea explicitly asks for visible copy. If it does, translate the intended meaning and render concise, correctly spelled ENGLISH only; never render non-English or pseudo-language lettering.
...
- Do not generate non-English or pseudo-language lettering. If the customer explicitly requests a new word-based mark, translate the intended meaning and use correctly spelled English only; keep an exact supplied proper brand name unchanged.
...
- Do NOT invent random words, fake prices, fake product names, fake letters, fake buttons, or fake website copy.
- Do NOT add any new typography unless the user's request explicitly asks for text in the marketing image. If requested, translate the intended meaning and render only concise, correctly spelled ENGLISH. Never synthesize Arabic, Korean, Cyrillic, Chinese, another non-English script, or pseudo-language glyphs.
- If any part of the actual website UI/screenshot is shown, treat its existing text as protected source pixels. Do not redraw, translate, correct, restyle, or replace it. If exact pixel preservation is not possible, omit/de-emphasize the text-bearing UI and let the product/visual carry the ad.
...
export async function generateCinematicSceneImage(
  ...
  const captionInstruction = onScreenCopy?.trim()
    ? `Interpret the meaning of this requested caption, translate it when needed, and render a concise correctly spelled ENGLISH caption in clean premium typography: "${onScreenCopy.trim()}". Do not reproduce non-English script and do not add any other text.`
...
- If a real captured UI screen is shown inside a device mockup, treat its text as protected source pixels and keep the layout recognizable. Never redraw the lettering; if exact preservation is not possible, frame the screen wider or omit it.
- Never synthesize non-English or pseudo-language lettering. Any explicitly requested new visible copy must be concise, correctly spelled English only.
