...
/**
 * Re-index stored scene references against the exact capture buffers loaded
 * for rendering. Planning may reduce/reorder the selected capture IDs, so the
 * planner's original numeric indexes are not safe to send to the media model
 * after a reload. Stable filenames are the source of truth.
 */
export function remapStoryboardScenesToCaptures(
  storyboard: Pick<Storyboard, 'scenes' | 'sceneCaptureIds'>,
  renderCaptureIds: string[]
): StoryboardScene[] {
  const indexById = new Map(renderCaptureIds.map((id, index) => [id, index]));
  return (storyboard.scenes ?? []).map((scene, sceneIndex) => {
    const stableIds = storyboard.sceneCaptureIds?.[sceneIndex];
    if (!stableIds) {
      return {
        ...scene,
        sourceIndices: (scene.sourceIndices ?? []).filter(
          (index) => Number.isInteger(index) && index >= 0 && index < renderCaptureIds.length
        )
      };
    }
    const sourceIndices = stableIds
      .map((id) => indexById.get(id))
      .filter((index): index is number => index !== undefined)
      .filter((index, position, values) => values.indexOf(index) === position);
    return { ...scene, sourceIndices };
  });
}
...
const STORYBOARD_SCHEMA = {
  ...
  properties: {
    concept: {
      ...
      description: 'One-sentence production concept grounded in the captured website assets and the selected mode.'
    },
    vibe: {
      type: 'STRING' as const,
      description: 'The pacing and editorial mood.'
    },
    ideas: {
      type: 'ARRAY' as const,
      ...
      items: {
        ...
        properties: {
          ...
          sceneType: {
            ...
            description:
              'Narrative role of the scene. For video, "interaction" means the AI should perform a real visible action grounded by before/after captures when available. Photos normally use hook/feature/cta.'
          },
          shotDescription: {
            type: 'STRING' as const,
          ...
          sourceIndices: {
            ...
            description:
              'CUSTOM/SCENARIO VIDEO: up to 3 zero-based CAPTURE indexes may be used as Veo asset references for subject/product/place/style continuity. WEBSITE VIDEO: normally 1 CAPTURE index is the starting frame; interaction scenes may use 2 indexes ordered before then after. PHOTOS: use an empty array.'
          },
          composition: {
            type: 'STRING' as const,
            enum: ['single', 'sequence', 'split', 'triple'],
            description:
              'Legacy compatibility field. Use single for current AI-video and photo planning; final motion is generated by the AI video provider.'
          },
          motion: {
            type: 'STRING' as const,
            enum: ['push_in', 'pull_out', 'pan_left', 'pan_right', 'pan_up', 'pan_down', 'static'],
            description:
              'Legacy compatibility field. Use static; describe real generated motion in shotDescription instead.'
          },
          focusX: {
          ...
          transition: {
            type: 'STRING' as const,
            description:
              'Narrative handoff intention only. The application does not create screenshot transitions as a substitute for AI-generated motion.'
          }
        },
        required: [
          'sceneNumber',
...
export const GENERATIVE_VIDEO_MODES = [
  'video',
  'both',
  'demo',
  'tutorial',
...
/** Server-only direction silently combined with every customer request. */
export const MASTER_CREATIVE_DIRECTOR_SYSTEM = `
HIDDEN MASTER CREATIVE-DIRECTOR SYSTEM — APPLY TO EVERY FEATURE:
- The customer's complete prompt is the creative contract and highest-priority art direction. Extract and preserve every compatible requirement: subject/product, action/story, environment, mood, palette, lighting, camera, pacing, sound/dialogue, text request, exclusions, format and ending. Never replace a specific request with a generic template.
- The feature-specific master direction controls professional execution, not the customer's intent. Fill only choices the customer left open. When directions appear to conflict, preserve the customer's requested meaning and use the master rules to make it coherent, beautiful, safe and technically achievable.
- Treat supplied website screenshots and uploaded images as actual visual evidence, not inspiration text. Preserve the identity of real products, people, brands, logos, UI and places. Never substitute a generic look-alike when a reference exists.
- Choose one strong central creative idea and one consistent visual bible: world, color palette, lighting direction, lens/camera language, material treatment, motion rhythm, performance style and sound world. Every beat/image must belong to it while still adding new information.
...
- Treat existing text inside a capture as protected source pixels. Keep that referenced text plane stable instead of directing the visual model to retype, translate, correct or redesign it.
...
- Never direct the visual model to synthesize Arabic, Korean, Cyrillic, Chinese or pseudo-language lettering. New visible copy, only when the customer explicitly requests it, must be short and correctly spelled ENGLISH only; real non-English source text remains protected reference imagery.
...
export function buildFallbackStoryboard(input: StoryboardInput): Storyboard {
  ...
  const actualCaptureCount =
    input.referenceCaptures?.length ||
    Number(Boolean(input.screenshotBase64)) + Number(Boolean(input.fullPageScreenshotBase64));
  ...
  const scenes = Array.from({ length: sceneCount }, (_, index): StoryboardScene => {
    ...
    const longFormDirection =
      !isPhotos && sceneCount > directions.length
        ? textOnlyStudio
    ...
    return {
      ...
      shotDescription: feature
        ...
        : longFormDirection,
      sourceIndices:
        isPhotos || textOnlyStudio
          ? []
          : isStudioVideo
            ? Array.from(
                { length: Math.min(3, actualCaptureCount) },
                (_, offset) => (index + variantSeed + offset) % actualCaptureCount
              )
      ...
      focusY: isPhotos
        ? 0.5
        : isFirst
          ? 0.22 + (variantSeed % 10) / 100
          : isLast
            ? 0.18 + (variantSeed % 14) / 100
            : 0.34 + ((variantSeed + index * 11) % 34) / 100,
      onScreenCopy: '',
      transition: isLast
        ? ''
        : 'Continue naturally into the next timeline beat of the same film without a visual or identity reset.'
    };
  });
...
export function buildStoryboardPrompt(input: StoryboardInput): PlannerPrompt {
  ...
  if (labeledCaptures.length > 0) {
    labeledCaptures.forEach((capture, index) => {
      ...
      captureParts.push({
        inlineData: { mimeType: 'image/jpeg', data: capture.base64 }
      });
    });
  } else {
    if (screenshotBase64) {
    ...
    if (fullPageScreenshotBase64) {
      captureParts.push({
        text: `CAPTURE ${screenshotBase64 ? 1 : 0}: Homepage full-page screenshot`
      });
      captureParts.push({
        inlineData: { mimeType: 'image/jpeg', data: fullPageScreenshotBase64 }
      });
    }
  }
  const captureCount =
    labeledCaptures.length || Number(Boolean(screenshotBase64)) + Number(Boolean(fullPageScreenshotBase64));
...
${
  isStudioVideo
    ? `PROJECT TYPE: ${isProductVideo ? 'Product video' : mode === 'talking-scene' ? 'Talking scene' : 'Independent AI video'}
WORKING TITLE: ${pageTitle}`
    : isPhotos && isUploadProject
      ? `PROJECT TYPE: Product / reference photo campaign
WORKING TITLE: ${pageTitle}`
      : `SITE: ${siteUrl}
BRAND: ${brandName}
...
export async function generateStoryboard(input: StoryboardInput & { jobId: string }): Promise<StoryboardResult> {
  const { mode, creativeBrief, aspectRatio, outputQuality, frameRate } = input;
  const { prompt, captureParts, captureCount, sceneCount, variantSeed, isPhotos, isDemo } =
    buildStoryboardPrompt(input);
  const isStudioVideo = ['custom', 'ai-video', 'product-video', 'talking-scene'].includes(mode);
  const textOnlyStudio = ['custom', 'ai-video', 'talking-scene'].includes(mode) && captureCount === 0;
  const model = storyboardModelName();
  console.info(
    `[storyboard] model=${model} mode=${mode} variation=${variantSeed} captures=${captureCount} scenes=${sceneCount}`
  );
  ...
  try {
      ...
      task: () =>
        client.models.generateContent({
          ...
          config: {
            ...
            responseSchema: STORYBOARD_SCHEMA as Parameters<typeof client.models.generateContent>[0]['config'] extends {
              responseSchema?: infer T;
            }
              ? T
              : never,
            temperature: 0.72
    ...
    const parsedBeatDurations = isPhotos
      ? Array(sceneCount).fill(0)
      : timelineBeatDurations(parsed.targetDurationSeconds, sceneCount);
    ...
    if (!isPhotos && !textOnlyStudio) {
      parsed.scenes = parsed.scenes.map((scene, index) => ({
        ...
        sourceIndices: scene.sourceIndices?.length
          ...
          : isStudioVideo
            ? Array.from(
                { length: Math.min(3, captureCount) },
                (_, offset) => (index + variantSeed + offset) % captureCount
              )
        ...
        parsed.scenes = parsed.scenes.map((scene, index) => {
          ...
          if (primary < 0 || usedPrimary.has(primary)) {
            const unused = Array.from(
              { length: captureCount },
              (_, offset) => (variantSeed + index + offset) % captureCount
