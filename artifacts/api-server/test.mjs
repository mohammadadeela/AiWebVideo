...
async function run() {
  ...
  await esbuild({
    ...
    bundle: true,
    format: "esm",
    // Some CommonJS dependencies (notably nodemailer) use dynamic built-in
    // requires. Each test bundle runs in a new process, so the parent process's
    // global require does not carry over; inject a local ESM-compatible require
    // into every generated test file.
    banner: {
    ...
    outExtension: { ".js": ".mjs" },
    logLevel: "warning",
    external: ["*.node", "@google/*", "express", "firebase-admin", "jsonwebtoken", "playwright", "pg", "pg-native", "pino", "pino-http", "pino-pretty"],
    sourcemap: "inline",
  });
