...
function withRenderProfile(profile: string | undefined, run: () => void) {
  const previous = process.env.AI_VIDEO_RENDER_PROFILE;
  if (profile === undefined) delete process.env.AI_VIDEO_RENDER_PROFILE;
  else process.env.AI_VIDEO_RENDER_PROFILE = profile;
  try { run(); }
  finally {
    if (previous === undefined) delete process.env.AI_VIDEO_RENDER_PROFILE;
    else process.env.AI_VIDEO_RENDER_PROFILE = previous;
  }
}
