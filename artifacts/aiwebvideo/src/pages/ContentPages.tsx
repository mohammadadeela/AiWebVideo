...
const features = [
  ['Multi-page website capture', 'Captures key pages after they finish loading, plus a smooth-scroll recording and brand details.'],
  ['Seven production modes', 'Create promos, how-to tutorials, purchase walkthroughs, feature tours, SaaS demos, photos, or combined campaigns.'],
  ['Creative direction in chat', 'Choose format, duration, mood, features, sound, and a detailed custom brief in plain language.'],
...
export function FeaturesPage() {
  return <Shell eyebrow="Product" title="A complete website content studio" intro="From capture to final delivery, every step is designed for marketers, founders, agencies, and online stores that need polished content without a traditional production workflow."><section><div className="mx-auto grid max-w-6xl gap-4 px-5 py-16 md:grid-cols-2 lg:grid-cols-3">{features.map(([title, body], index) => <article key={title} className="rounded-3xl border border-border bg-panel p-6"><span className="font-utility text-xs text-violet">0{index + 1}</span><h2 className="mt-5 font-display text-lg font-semibold text-text-primary">{title}</h2><p className="mt-2 text-sm leading-relaxed text-text-muted">{body}</p></article>)}</div></section></Shell>;
}

const steps = [
  ['Paste a public website URL', 'The secure capture studies the real pages, screenshots, scrolling behavior, brand colors, and visible features.'],
  ['Choose the production', 'Pick a promo, tutorial, purchase guide, feature tour, SaaS demo, photo set, or a complete bundle.'],
  ['Direct the creative', 'Set duration and format, then explain the audience, scenes, mood, products, messages, and anything to avoid.'],
...
export function TermsPage() {
  return <LegalPage title="Terms of service" intro="These terms describe the basic rules for using AiWebVideo and its production features." sections={[
