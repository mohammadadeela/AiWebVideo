...
function UserDetailsModal({ details, loading, onClose }: { details: { user: Row; subscriptions: Row[]; payments: Row[]; credits: Row[]; productions: Row[] } | null; loading: boolean; onClose: () => void }) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="User details" className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto bg-black/70 p-3 backdrop-blur-sm sm:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div className="my-6 w-full max-w-6xl overflow-hidden rounded-[28px] border border-white/10 bg-[#111116] shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-[#111116]/95 px-5 py-4 backdrop-blur-xl">
...
export function AdminPage() {
  ...
  return <div className="min-h-screen bg-bg lg:flex">
    <aside className="border-b border-white/[.09] bg-[#0d0d10] p-4 lg:sticky lg:top-0 lg:h-screen lg:w-64 lg:border-b-0 lg:border-r">
      <div className="flex items-center justify-between"><Link href="/"><Wordmark /></Link><span className="text-xs font-semibold text-text-muted">Admin</span></div>
      <nav className="mt-7 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-1">{tabs.map((item) => <button key={item.id} type="button" onClick={() => selectTab(item.id)} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-left text-xs font-semibold transition ${tab === item.id ? 'bg-white/[.06] text-text-primary' : 'text-text-muted hover:bg-white/[.035] hover:text-text-primary'}`}><item.icon size={16} />{item.label}</button>)}</nav>
