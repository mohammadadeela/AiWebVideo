...
import { Activity, AlertTriangle, CheckCircle2, Cpu, Database, DollarSign, FileVideo, KeyRound, LayoutDashboard, RefreshCw, Search, Settings2, ShieldCheck, SlidersHorizontal, Users, Upload, Save, GalleryVerticalEnd, type LucideProps } from 'lucide-react';
import { Button } from '@/components/ui/app-button';
import { Switch } from '@/components/ui/switch';
import { Wordmark } from '@/components/ui/Wordmark';
import {
  fetchAdminAudit, fetchAdminJobs, fetchAdminOverview, fetchAdminUsers, fetchMe,
  saveAdminSettings, updateAdminJob, updateAdminUser, saveMarketingSettings, uploadMarketingAsset,
  type AdminSettings, type ProviderChoice, type MarketingSettings,
} from '@/lib/api-client';
...
function VideoCostMatrix({ rows }: { rows: Row[] }) {
  return <section className="rounded-3xl border border-border bg-panel p-5">
    <div><h2 className="font-semibold text-text-primary">Every Gemini video length · cost and customer credits</h2><p className="text-xs text-text-dim">Exact 8-second production steps from 8 seconds to 4 minutes. Provider estimates exclude small planning/TTS token costs; completed jobs record their exact total above.</p></div>
    <div className="mt-4 max-h-[520px] overflow-auto rounded-2xl border border-border">
      <table className="w-full min-w-[980px] text-left text-xs">
        <thead className="sticky top-0 z-10 bg-panel-alt text-text-dim"><tr><th className="p-3">Length</th><th>Scenes</th><th>Fast 1080p</th><th>Fast 4K</th><th>Standard 1080p</th><th>Standard 4K</th><th>Customer 1080p</th><th>Customer 4K</th><th className="pr-3">Extras</th></tr></thead>
        <tbody className="divide-y divide-border">{rows.map((row) => <tr key={number(row.seconds)}><td className="p-3 font-semibold text-text-primary">{number(row.seconds) >= 60 ? `${Math.floor(number(row.seconds) / 60)}m ${number(row.seconds) % 60}s` : `${number(row.seconds)}s`}</td><td className="text-text-muted">{number(row.scenes)}</td><td className="text-mint">${number(row.geminiFast1080Usd).toFixed(2)}</td><td className="text-mint">${number(row.geminiFast4kUsd).toFixed(2)}</td><td className="text-text-muted">${number(row.geminiStandard1080Usd).toFixed(2)}</td><td className="text-text-muted">${number(row.geminiStandard4kUsd).toFixed(2)}</td><td className="font-utility text-text-primary">{number(row.userCredits1080)} credits</td><td className="font-utility text-text-primary">{number(row.userCredits4k)} credits</td><td className="pr-3 text-text-dim">Voice +{number(row.narrationCredits)} · Video+photos +{number(row.videoAndPhotosExtraCredits)}</td></tr>)}</tbody>
      </table>
    </div>
  </section>;
}
...
export function AdminPage() {
  ...
  async function uploadLandingFile(index: number, file: File, target: 'url' | 'posterUrl') {
    ...
    try {
      ...
      setMarketing((current) => current ? ({ ...current, videos: { showcase: current.videos.showcase.map((video, i) => i === index ? { ...video, [target]: uploaded.url } : video) } }) : current);
      setDirty(true); setMessage('Upload ready. Use Save landing page to publish it.');
  ...
  return <div className="min-h-screen bg-bg lg:flex">
    ...
    <main className="min-w-0 flex-1 p-4 sm:p-7 lg:p-10">
      ...
      {tab === 'overview' && <div className="mt-7 space-y-6">
        ...
        <section className="rounded-3xl border border-border bg-panel p-5"><div><h2 className="font-semibold text-text-primary">Provider cost by process · this month</h2><p className="text-xs text-text-dim">Recorded from each completed Gemini or GPU generation step.</p></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[680px] text-left text-xs"><thead className="text-text-dim"><tr><th className="pb-3">Provider / model</th><th>Process</th><th>Usage</th><th>Events</th><th>Cost</th></tr></thead><tbody className="divide-y divide-border">{costBreakdown.map((row, index) => <tr key={`${text(row.provider)}-${text(row.operation)}-${index}`}><td className="py-3 text-text-primary">{text(row.provider)} · {text(row.model)}</td><td className="text-text-muted">{text(row.operation).replaceAll('_', ' ')}</td><td className="text-text-muted">{number(row.quantity).toFixed(2)} {text(row.unit)}</td><td className="text-text-muted">{number(row.events)}</td><td className="font-semibold text-mint">${number(row.cost).toFixed(4)}</td></tr>)}</tbody></table>{!costBreakdown.length && <p className="py-8 text-center text-sm text-text-dim">Costs will appear after the next generation.</p>}</div></section>
      ...
      {tab === 'landing' && marketing && <section className="mt-7 space-y-5"><div className="rounded-3xl border border-violet/25 bg-gradient-to-br from-violet/10 to-gold/5 p-5"><p className="text-xs font-semibold uppercase tracking-[.16em] text-violet">Live preview controls</p><h2 className="mt-2 font-display text-2xl font-bold text-text-primary">Show visitors what customers create</h2><p className="mt-2 max-w-2xl text-sm text-text-muted">Upload a small portrait video or paste a YouTube, Vimeo, or direct video URL. The short overlay explains the result before the visitor presses play.</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-xs font-semibold text-text-primary">Section heading<input value={marketing.heading} onChange={(event) => { setMarketing({ ...marketing, heading: event.target.value }); setDirty(true); }} className="mt-2 w-full rounded-xl border border-border bg-bg px-3 py-2.5 text-sm" /></label><label className="text-xs font-semibold text-text-primary">Section description<textarea rows={2} value={marketing.description} onChange={(event) => { setMarketing({ ...marketing, description: event.target.value }); setDirty(true); }} className="mt-2 w-full resize-none rounded-xl border border-border bg-bg px-3 py-2.5 text-sm" /></label></div></div><div className="grid gap-5 xl:grid-cols-3">{marketing.videos.showcase.map((video, index) => <article key={video.id} className="rounded-3xl border border-border bg-panel p-5"><div className="flex items-center justify-between"><div><p className="font-semibold text-text-primary">Example {index + 1}</p><p className="text-[10px] text-text-dim">Portrait cards stay compact on the homepage</p></div>{video.url && <button type="button" onClick={() => { setMarketing({ ...marketing, videos: { showcase: marketing.videos.showcase.map((item, i) => i === index ? { ...item, url: null } : item) } }); setDirty(true); }} className="text-xs text-pink">Remove</button>}</div>{video.url ? <video src={video.url} poster={video.posterUrl ?? undefined} controls className="mx-auto mt-4 aspect-[9/16] max-h-60 rounded-2xl bg-black object-cover" /> : <div className="mt-4 flex aspect-[9/16] max-h-60 items-center justify-center rounded-2xl border border-dashed border-violet/30 bg-bg/50 text-xs text-text-dim">No video yet</div>}<div className="mt-4 space-y-3"><label className="block text-xs text-text-muted">Video URL<input value={video.url ?? ''} placeholder="YouTube, Vimeo, or direct MP4 URL" onChange={(event) => { setMarketing({ ...marketing, videos: { showcase: marketing.videos.showcase.map((item, i) => i === index ? { ...item, url: event.target.value || null } : item) } }); setDirty(true); }} className="mt-1.5 w-full rounded-xl border border-border bg-bg px-3 py-2.5 text-xs" /></label><label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-violet/30 bg-violet/10 px-3 py-2.5 text-xs font-semibold text-violet"><Upload size={14} /> Upload video<input type="file" accept="video/mp4,video/webm,video/quicktime" className="hidden" disabled={busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadLandingFile(index, file, 'url'); event.currentTarget.value = ''; }} /></label><label className="block text-xs text-text-muted">Small label<input value={video.eyebrow ?? ''} placeholder="Made by a customer" onChange={(event) => { setMarketing({ ...marketing, videos: { showcase: marketing.videos.showcase.map((item, i) => i === index ? { ...item, eyebrow: event.target.value || null } : item) } }); setDirty(true); }} className="mt-1.5 w-full rounded-xl border border-border bg-bg px-3 py-2.5 text-xs" /></label><label className="block text-xs text-text-muted">Caption<input value={video.caption ?? ''} placeholder="Fashion store launch video" onChange={(event) => { setMarketing({ ...marketing, videos: { showcase: marketing.videos.showcase.map((item, i) => i === index ? { ...item, caption: event.target.value || null } : item) } }); setDirty(true); }} className="mt-1.5 w-full rounded-xl border border-border bg-bg px-3 py-2.5 text-xs" /></label><label className="block text-xs text-text-muted">Text over the video<textarea rows={2} value={video.overlayText ?? ''} placeholder="Created from a real website in minutes" onChange={(event) => { setMarketing({ ...marketing, videos: { showcase: marketing.videos.showcase.map((item, i) => i === index ? { ...item, overlayText: event.target.value || null } : item) } }); setDirty(true); }} className="mt-1.5 w-full resize-none rounded-xl border border-border bg-bg px-3 py-2.5 text-xs" /></label></div></article>)}</div></section>}

      {(tab === 'users' || tab === 'jobs') && <div className="mt-7 flex flex-col gap-3 sm:flex-row"><div className="relative flex-1"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-dim" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={tab === 'users' ? 'Search users by email' : 'Search projects, URLs, or users'} className="w-full rounded-xl border border-border bg-panel py-3 pl-9 pr-3 text-base text-text-primary outline-none focus:border-violet/50 sm:text-xs" /></div>{tab === 'jobs' && <select value={jobStatus} onChange={(event) => setJobStatus(event.target.value)} className="rounded-xl border border-border bg-panel px-4 py-3 text-xs text-text-primary"><option value="all">All statuses</option><option value="rendering">Rendering</option><option value="done">Completed</option><option value="failed">Failed</option><option value="capturing">Capturing</option></select>}</div>}
