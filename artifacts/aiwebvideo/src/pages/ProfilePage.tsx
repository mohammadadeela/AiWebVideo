...
export function ProfilePage() {
  useSeo({ title: 'Your account', description: 'Manage your AiWebVideo account, plan, and credits.', path: '/profile', noindex: true });
  ...
  async function billing(action: 'portal' | 'topup', provider: 'stripe' | 'paypal' = 'stripe') {
    setBusy(true); setError(null);
    try {
      const target = action === 'portal' ? (await openBillingPortal()).portalUrl : (await startTopup(provider)).checkoutUrl;
            ...
            <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
              ...
              <section className="rounded-3xl border border-border bg-panel p-6">
                ...
                <div className="mt-4 space-y-2">
                  {me && me.creditsBalance < 100_000 && <Button variant="secondary" className="w-full" disabled={busy} onClick={() => void billing('topup')}>Buy 100 credits</Button>}
                  {me && me.creditsBalance < 100_000 && <Button variant="ghost" className="w-full" disabled={busy} onClick={() => void billing('topup', 'paypal')}>Buy 100 credits with PayPal</Button>}
