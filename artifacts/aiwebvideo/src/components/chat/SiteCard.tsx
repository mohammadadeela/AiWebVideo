          ...
          <div className="grid grid-cols-2 gap-2">
            {allShots.map((page, index) => (
              <button
                ...
                onClick={() => setPreview({ url: page.screenshotUrl, title: page.title || hostname })}
              ...
              >
                <img src={page.screenshotUrl} alt={`Saved capture of ${page.title || hostname}`} className="aspect-video w-full object-cover object-top" />
      ...
      <div className="flex items-center gap-3">
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-panel-alt border border-border overflow-hidden">
          {metadata.logoUrl ? (
            ...
            <span className="text-text-dim text-xs font-utility">{hostname.slice(0, 2).toUpperCase()}</span>
          )}
        </div>
