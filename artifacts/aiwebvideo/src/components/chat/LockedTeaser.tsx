  ...
  return (
    <div className="w-full max-w-sm animate-fade-in-up">
      <div className="relative overflow-hidden rounded-xl border border-violet/40 bg-panel-alt">
        <div className="relative aspect-video overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
            <span className="rounded-full border border-white/25 bg-black/55 px-3 py-1 font-utility text-[10px] uppercase tracking-widest text-white">Saved preview</span>
            <p className="px-6 text-center text-sm font-semibold text-white drop-shadow">
              Your website source for {hostname} is ready for a {durationSeconds}s production
            </p>
          </div>

          {/* Bottom info strip */}
          <div className="absolute inset-x-0 bottom-0 flex items-center justify-between px-3 py-2 text-[11px] text-white/90">
            <span className="font-utility rounded bg-black/60 px-2 py-0.5">{sceneCount} scenes when planned</span>
      ...
      <p className="mt-1.5 text-center text-[11px] text-text-dim">
        Credits are checked before any new paid AI or provider generation step. Failed paid generations are refunded automatically.
      </p>
    </div>
  );
}
