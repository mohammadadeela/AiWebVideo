    ...
    <div className="w-full max-w-sm space-y-3 animate-fade-in-up">
        ...
        <div className="space-y-2">
          <div className="relative overflow-hidden rounded-xl border border-border bg-black">
            ...
            {activeVideo.downloadable && (
              <button
                type="button"
                onClick={() => void downloadFile(activeVideo)}
                aria-label="Download video"
                title="Download video"
                className="absolute right-2.5 top-2.5 flex h-9 w-9 items-center justify-center rounded-full border border-white/15 bg-black/55 text-white backdrop-blur transition hover:bg-black/75 focus-visible:outline focus-visible:outline-2 focus-visible:outline-violet"
              >
                <Download size={16} />
              </button>
            )}
      ...
      {displayPhotos.length > 0 && (
        <div className="grid grid-cols-2 gap-2">
          {displayPhotos.slice(0, 4).map((photo) => (
            <div key={photo.id} className="group relative aspect-square overflow-hidden rounded-lg border border-border bg-panel-alt">
              ...
              {photo.downloadable && (
                <button
                  type="button"
                  onClick={(event) => { event.stopPropagation(); void downloadFile(photo); }}
                  aria-label="Download photo"
                  title="Download photo"
                  className="absolute right-1.5 top-1.5 flex h-7 w-7 items-center justify-center rounded-full border border-white/15 bg-black/55 text-white backdrop-blur transition hover:bg-black/75 focus-visible:outline focus-visible:outline-2 focus-visible:outline-violet"
                >
                  <Download size={13} />
                </button>
              )}
      ...
      {anyLocked ? (
        ...
        assets.filter((asset) => asset.downloadable).length > 1 && (
          <Button variant="secondary" size="md" className="w-full" onClick={downloadAll}><Images size={15} /> Download all files</Button>
        )
      )}
      ...
      {activePhoto && createPortal(
        <div role="dialog" aria-modal="true" aria-label="Photo preview" onClick={() => setActivePhoto(null)} className="fixed inset-0 z-[100] flex cursor-zoom-out items-center justify-center bg-black/55 p-4 backdrop-blur-sm">
          ...
          <div className="flex max-h-[58vh] min-h-52 items-center justify-center bg-black/70 p-3 sm:p-5">
            <div className="relative max-h-[52vh] overflow-hidden rounded-lg">
              ...
              {activePhoto.downloadable && (
                <button
                  type="button"
                  onClick={() => void downloadFile(activePhoto)}
                  aria-label="Download photo"
                  title="Download photo"
                  className="absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-full border border-white/15 bg-black/55 text-white backdrop-blur transition hover:bg-black/75"
                >
                  <Download size={16} />
                </button>
              )}
