...
export function GenerationCanvas({
  ...
  const showReferenceRail = productionKind === "website-video" && (status === "capturing" || referenceItems.length > 0);
  ...
  return (
    ...
    >
      ...
      <div className="relative px-5 py-5 sm:px-7 sm:py-6">
        ...
        <div className="mt-6">
          <ol className="grid grid-cols-4 gap-2" aria-label="Production stages">
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.08fr)_minmax(320px,.92fr)] lg:items-start lg:gap-7">
            {showReferenceRail && (
              <div className="min-w-0 rounded-2xl border border-white/[.07] bg-white/[.025] p-4">
                ...
                <div className="chat-scroll mt-3 flex gap-3 overflow-x-auto pb-1">
                  {(referenceItems.length ? referenceItems : Array.from({ length: 4 }).map((_, index) => ({ id: `placeholder-${index}`, url: "", title: `Capture ${index + 1}` } as CaptureMediaItem))).map((item) => {
            ...
            <div className={`relative overflow-hidden rounded-[24px] border border-white/[.08] bg-[#100c1c] p-4 sm:p-5 ${showReferenceRail ? "" : "lg:col-span-2"}`}>
          ...
          <div className="mt-6 rounded-[24px] border border-white/[.08] bg-white/[.02] p-4 sm:p-5">
            <div className="flex items-center gap-2 text-mint">
              <Activity size={13} aria-hidden="true" />
              <span className="font-utility text-[9px] font-semibold uppercase tracking-[.19em]">{phaseLabel}</span>
            </div>
            <h3 className="mt-2 max-w-3xl font-display text-2xl font-bold leading-[1.1] tracking-[-.025em] text-white sm:text-[30px]">{copy.active}</h3>
            {status === "capturing" && (
              <span className="sr-only" role="status" aria-live="polite">{liveMessage}</span>
            )}
            {status !== "capturing" && (
              <div className="mt-3 flex max-w-3xl flex-wrap items-start gap-2.5">
            ...
            <div className={`${status === "capturing" ? "mt-3" : "mt-5"} rounded-2xl border border-white/[.07] bg-white/[.028] p-4`}>
              <div className="flex items-center gap-2 text-mint">
                <Activity size={13} aria-hidden="true" />
