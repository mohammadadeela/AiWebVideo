...
const STAGES = new Set<JobWorkflowState["stage"]>([
  "capturing",
  "preview_ready",
...
const MODES = new Set<JobWorkflowState["mode"]>([
  "video",
  "photos",
  "icon",
  "both",
...
const ASPECT_RATIOS = new Set<JobWorkflowState["aspectRatio"]>([
  "16:9",
  "9:16",
  "1:1",
]);
...
export function normalizeJobWorkflow(value: unknown): JobWorkflowState | null {
  ...
  if (
    !STAGES.has(raw.stage as JobWorkflowState["stage"]) ||
    !MODES.has(raw.mode as JobWorkflowState["mode"])
  )
    return null;
  const seconds =
  ...
  return {
    ...
    featuresText:
      typeof raw.featuresText === "string"
        ? raw.featuresText.slice(0, 2000)
        : null,
    creativeBrief:
      typeof raw.creativeBrief === "string"
        ? raw.creativeBrief.slice(0, 8000)
        : null,
    ...
    requestedDurationSeconds:
      raw.requestedDurationSeconds === "auto"
        ? "auto"
        : typeof raw.requestedDurationSeconds === "number" &&
...
export function saveLocalJobWorkflow(jobId: string, state: JobWorkflowState) {
  try {
    localStorage.setItem(`${PREFIX}${jobId}`, JSON.stringify(state));
  } catch {
...
export function loadLocalJobWorkflow(jobId: string): JobWorkflowState | null {
  try {
    const value = normalizeJobWorkflow(
      JSON.parse(localStorage.getItem(`${PREFIX}${jobId}`) ?? "null"),
    );
