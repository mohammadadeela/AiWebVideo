...
Keep `GEMINI_VIDEO_MODEL=veo-3.1-fast-generate-preview` and
...
| `STRIPE_PRICE_SINGLE_8` | One-time | $2.99 | 14 |
| `STRIPE_PRICE_SINGLE_30` | One-time | $7.99 | 30 |
| `STRIPE_PRICE_SINGLE_60` | One-time | $17.99 | 62 |
...
The current subscription retail value is at least $0.2475 per credit. With the
margin-safe Veo 3.1 Fast model, 1080p uses one credit per generated second at
$0.12/sec provider cost, while 4K uses three credits per generated second at
$0.30/sec provider cost. Both clear the 2x gross provider-cost floor. If Veo
Standard is explicitly configured, the server automatically charges four
credits/sec for 1080p and five credits/sec for 4K. Re-check provider pricing
before changing plan prices, credit grants, or model settings.
